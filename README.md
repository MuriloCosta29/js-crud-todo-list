...
Como o botão "Apagar" é filho do elemento `<li>`, ao capturar o `innerText` da tarefa, a palavra "Apagar" vinha junto com o texto (ex: "Estudar Apagar"). Utilizei métodos de manipulação de String para limpar os dados antes de salvar:
...
Reforcei o uso de Event Delegation. Em vez de adicionar um "ouvinte" para cada botão criado (o que seria custoso para a performance), adicionei um único escutador no document. Utilizei a propriedade e.target para identificar se o clique veio de um botão com a classe .apagar e, a partir daí, remover o elemento pai (parentElement).
...
## 📌 Nota sobre o Código Fonte e Créditos
Optei por manter os comentários explicativos nos arquivos JavaScript para documentar meu raciocínio e facilitar futuras revisões.
